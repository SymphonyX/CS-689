...
In addition to the data provided by the BTS, we wanted to augment the feature set for flights by looking at weather patterns for each of the airports to determine the effect that weather might have on flight delays.
...
Detailed flight information was obtained from the forecast.io weather API \footnote{\url{http://www.forecast.io}}.
High level weather normals were obtained from the National Oceanic and Atmospheric Association (NOAA) \footnote{\url{www.getthelink.com}}.
In the Evaluation section we discuss the selection of features and which featured were included from these data sources for each task. 
...
We will evaluate our results in two ways: as a standard binary classification task and as a ranking task that is in line with our stated problem. 
The task can be looked at as a binary classification problem: Will a particular flight be delayed or not?
In this case, we will use accuracy of our model classifications as the evaluation metric.
Another metric we can use is a ranking accuracy metric.
We will consider the following question: Given two flights from the same location to the same destination, which flight has the higher probability of being delayed?
Our evaluation metric will then be the percentage of flight pairs where the model correctly ranks the flights in terms of probability of being delayed.
\textcolor{red}{
    \begin{itemize}
