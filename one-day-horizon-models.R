...
split.train.test <- function(df) {
  ...
  return(list(train=df[df$pair_id %in% train.pairs, ],
              test=df[df$pair_id %in% test.pairs, ]))
}
...
dat <- read.csv("../data/flight-pairs.csv", nrows=5000)
...
report.results("Random forest", class.probs)
