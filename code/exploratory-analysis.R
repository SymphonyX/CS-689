...
carrier.dat$CarrierName <- str_replace(carrier.dat$Description, "\\(Merged.+", "")
carrier.dat$Name <- str_replace_all(carrier.dat$CarrierName, "\\s", "\n")
carrier.dat$Name <- factor(carrier.dat$Name, levels=carrier.dat$Name[order(carrier.dat$estmean)])
g <- ggplot(carrier.dat, aes(x=Name, y=estmean, 
                             ymin=estmean-estsd, ymax=estmean+estsd, color=Name)) + 
